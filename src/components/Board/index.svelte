<script>
	...
	import { gamePaused, gameStore } from '@sudoku/stores/game';
			...
			{#each $gameStore.grid as row, y}
				{#each row as value, x}
					<Cell {value}
					      ...
					      userNumber={$gameStore.puzzleGrid[y][x] === 0}
					      ...
					      conflictingNumber={$settings.highlightConflicting && $gameStore.puzzleGrid[y][x] === 0 && $gameStore.invalidCells.includes(x + ',' + y)} />
				{/each}
			{/each}
